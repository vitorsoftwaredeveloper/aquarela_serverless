...
import { somarDiasBrasil } from "../../utils/date";
import { IMensalidade } from "../../types/mensalidades";

/**
 * Marca `inadimplenteDesde` nas mensalidades não pagas cuja carência já
 * venceu: `vencimento + configPrecos.inadimplencia.diasCarencia`. A conta é
 * por mensalidade — o `diaVencimento` de cada criança manda, não existe corte
 * de calendário comum a todo mundo. Só grava — nunca desmarca: a limpeza do
 * campo é responsabilidade exclusiva do caminho de pagamento (PIX, webhook ou
 * manual), que roda na mesma transação da baixa. Idempotente por construção:
 * o filtro já exclui quem tem o campo setado, então reexecuções no mesmo dia
...
export const marcarInadimplentesService = async (): Promise<{
  ...
  const { diasCarencia } = inadimplencia;
